        ...
        <v-card elevation="10">
            ...
            <v-row class="mx-1" align="center">
                <v-col cols="6" sm="6" md="4">
                    <h4 class="text-h4">Sub Category List</h4>
                </v-col>
            ...
            <v-data-table-server v-model:items-per-page="per_row" :headers="headers" :items="List"
                ...
                <template v-slot:item.name="{ item }">
                    <v-avatar color="blue" size="36" class="mr-2">
                        <v-img v-if="item.sub_category_url" :src="item.sub_category_url"></v-img>
                ...
                <template v-slot:item.actions="{ item }">
                    ...
                    <v-btn v-if="item.sub_category_type == 'yes'" @click="confirmDelete(item)" size="small"
                        variant="text" color="error" icon>
                        <TrashIcon />
                    </v-btn>
                </template>
            </v-data-table-server>
    ...
    <div class="ml-5">
        ...
        <v-dialog v-model="commonDialog" max-width="500" persistent scrollable>
            <v-card class="mx-auto p-3" elevation="2">
                <v-card-title>
                    <h3>{{ !selected_info ? "Sub Category Create" : 'Sub Category Edit' }}</h3>
                </v-card-title>
                ...
                <v-card-text>
                    <v-row>
                        ...
                        <v-col cols="12" class="text-center">
                            <v-file-input id="image-selected-empty" :clearable="false" v-model="form.image"
                                label="Choose Image" accept="image/*" variant="outlined" density="compact" hide-details
                                @change="onImageSelected" :show-prepend-icon="false">
                    ...
                    <v-row>
                        ...
                        <!-- sub_category_type Field -->
                        <v-col cols="12">
                            <v-select label="Delete Type" :items="['yes', 'no']" v-model="form.sub_category_type"
                                variant="outlined" density="compact"
                                :error-messages="v$.form.sub_category_type.$errors.map(e => e.$message)"
                                @blur="v$.form.sub_category_type.$touch()"></v-select>
                        </v-col>
...
export default {
    ...
    methods: {
        ...
        async getList() {
            ...
            await axios.post('/api/sub-category-list', params)
                .then(response => {
        ...
        onImageSelectedNew(e) {
            const file = e.target.files[0];
            const maxFileSize = 512000; // 500KB in bytes file.size < 2111775 2mb
            const validImageTypes = ["image/jpeg", "image/png", "image/jpg"];
            if (file && validImageTypes.includes(file.type)) {
                if (file.size <= maxFileSize) {
                    const fileSizeKB = (file.size / 1024).toFixed(2);
                    e.target.nextElementSibling.innerHTML = `${file.name} (${fileSizeKB} KB)`;
                    const reader = new FileReader();
                    reader.onloadend = () => { this.form.blackLogo = reader.result; };
                    reader.readAsDataURL(file);
                    this.blackLogo = URL.createObjectURL(file);
                } else {
                    this.$store.dispatch("globalState/errorSnackBar", "Please select a valid thumbnail with size less than 500kb");
                    e.target.value = "";
                    e.target.nextElementSibling.innerHTML = '';
                }
            } else {
                this.$store.dispatch("globalState/errorSnackBar", "Plz Select file this Extension (.jpg,.jpeg,.png,.gif,.ico)");
                e.target.value = "";
                e.target.nextElementSibling.innerHTML = '';
            }
        },
        ...
        openCommonDialog(info = null) {
            if (info) {
                ...
                this.image_url = info.sub_category_url;
                this.form = {
                    id: info.id,
                    name: info.name,
                    category_id: info.category_id,
                    sub_category_type: info.sub_category_type,
        ...
        async submitForm() {
            ...
            formData.append('sub_category_type', this.form.sub_category_type);
            ...
            const url = this.selected_info ? '/api/sub-category-update' : '/api/sub-category-create';
        ...
        async deleteInfo() {
            ...
            await axios.post('/api/sub-category-delete', params)
                .then(response => {
                    this.getList();
